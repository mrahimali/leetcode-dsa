...
### Note
While this approach is straightforward and easy to understand, it is not the most efficient solution for large input lists due to its quadratic time complexity. A more efficient solution would use a hash table to store the numbers and their indices, allowing for a time complexity of **O(n)**.